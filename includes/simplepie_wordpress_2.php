 ...
 */
define('SIMPLEPIE_PLUGINDIR_WEB', get_bloginfo('template_url') . '/' . SIMPLEPIE_PLUGINDIR_NAME); //edited: change path to themes-> define('SIMPLEPIE_PLUGINDIR_WEB', WP_CONTENT_WEB . '/plugins/' . SIMPLEPIE_PLUGINDIR_NAME);

/**
 * Default cache directory.
 */
define('SIMPLEPIE_CACHEDIR', SIMPLEPIE_PLUGINDIR . '/cache');
...
{
	update_option('simplepie_template', SIMPLEPIE_PLUGINDIR . '/templates/textrss.tmpl', 'The template to use for displaying feeds.');
}
...
{
	update_option('simplepie_date_format', 'j M y, g:ia', 'The default format for English dates.'); //edited: date format
}
...
{
	update_option('simplepie_truncate_feed_description', 300, 'Number of characters to shorten the text to.');
}

if (!get_option('simplepie_truncate_item_title'))
{
	update_option('simplepie_truncate_item_title', 50, 'Number of characters to shorten the text to.');
}

if (!get_option('simplepie_truncate_item_description'))
{
	update_option('simplepie_truncate_item_description', 250, 'Number of characters to shorten the text to.');
}
 ...
 */
//edited: don't show options page-> 
...
function SimplePie_Truncate($s, $length = 0)
{
    ...
    //edited: prevent funny char in title tooltip->$s = preg_replace('/(\s+)/', ' ', $s);

	// Avoid PHP 4.x bug. Only do this if we're on PHP5. http://bugs.php.net/25670
	//edited: do for both php4 and php5
	//if (SIMPLEPIE_PHP5)
	//{
		// Convert all HTML entities to their character counterparts.
		$s = htmlspecialchars($s, ENT_QUOTES);//edited for clean tootip-> $s = html_entity_decode($s, ENT_QUOTES, 'UTF-8');
	//}

	// Shorten the string to the number of characters requested, and strip wrapping whitespace.
	if ($length > 0 && strlen($s) > $length)
	{
		$s = trim(substr($s, 0, $length)) . '&hellip;'; //edited: valid xhtml-> $s = trim(substr($s, 0, $length)) . '. [&hellip;]';
	}
